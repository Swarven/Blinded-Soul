function AllianceBlacksmithQuest_OnGossipTalk(pUnit, event, player, pMisc)
   pUnit:GossipCreateMenu(50, player, 0)
   if (player:HasFinishedQuest(2000004) == true) then
     if (player:HasFinishedQuest(2000005) == false) then
   pUnit:GossipMenuAddItem(9, "Your the Blacksmith right?", 1, 0)
   end
   if (player:HasFinishedQuest(2000005) == true) then
   ...
   if (player:GetItemCount(142423) == 1) then
     pUnit:GossipMenuAddItem(3, "Can you fix this key?", 21, 0)
	 end
   if (player:HasFinishedQuest(2000019) == true) then
   if (player:GetItemCount(142422) == 1) then
...
function AllianceBlacksmithQuest_OnGossipSelect(pUnit, event, player, id, intid, code, pMisc)
if (intid == 1) then
...
if (intid == 5) then
   ...
   player:SendBroadcastMessage("Degor Stares at you angerly")
   player:SendAreaTriggerMessage("Degor Stares at you angerly")
end
...
if (intid == 2) then
if (player:GetItemCount(76844) == 1) then
--///////////////////////////////////--
...
--///////////////////////////////////--
    player:RemoveItem(76844,1)
	player:AddItem(76845,1)
...
if (intid == 20) then
   pUnit:SendChatMessage(12, 0, "Kill the Scourge Skeletons just beyond the gates of The Sanctuary. I can forge their weapons into a better one.")
player:GossipComplete()
end

if (intid == 21) then
player:RemoveItem(142423,1)
...
----------------------
RegisterUnitGossipEvent(9998805, 1, "AllianceBlacksmithQuest_OnGossipTalk")
RegisterUnitGossipEvent(9998805, 2, "AllianceBlacksmithQuest_OnGossipSelect")
